      ...
      <div className="w-full p-5 rounded-2xl  shadow-2xl">
        ...
        <form onSubmit={handleSubmit(onSubmit)}>
          ...
          <div class="form-control w-full">
            <label class="label text-sm font-semibold py-0 ml-2">
              <span class="label-text ">Email</span>
            </label>
            <input type="text" name='email' className={inputClass}  {...register("email",
              {
                required: { value: true, message: 'Email is required' },
                pattern: { value: /[A-Za-z0-9]+@[a-z]+\.[a-z]{2,3}/, message: 'Provide a valid email' }
              })} />
            <label class="label pt-1 ml-2">
              <span class="label-text-alt"></span>
              {(errors.email?.type === 'required' || errors.email?.type === 'pattern') && <span class="label-text-alt text-[12px] text-red-600">{errors.email.message}</span>}
            </label>
          </div>
          {/* password form */}
          <div class="form-control w-full">
            ...
            <label class="label pt-1 ml-2">
              <span class="label-text-alt"></span>
              {(errors.password?.type === 'required' || errors.password?.type === 'minLength') && <span class="label-text-alt text-[12px] text-red-600">{errors.password.message}</span>}
            </label>
          </div>
